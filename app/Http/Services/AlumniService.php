...
class AlumniService
{
    ...
    public function getAlumniListAllWithAdvanceFilter($request){
        ...
        ->orderBy('users.name', 'ASC')
        ->where('users.tenant_id', getTenantId())
        ->select('users.id','users.name','users.image','users.mobile','users.email','users.status','users.show_email_in_public','users.show_phone_in_public','batches.name as batch', 'passing_years.name as passing_year','alumnus.facebook_url','alumnus.twitter_url','alumnus.instagram_url','alumnus.city','alumnus.state','alumnus.zip','alumnus.country','alumnus.address','alumnus.file',DB::raw('max(user_membership_plans.expired_date) as expired_date'));
        ...
        return datatables($alumniData)
            ...
            ->addColumn('address', function ($data) {
              ...
              ->addColumn('file', function ($data) {
                ...
                if(!is_null($data->file)){
                    //getFilePath($data->file)
                  //  attach-download
                 //   return "<a href=".route('attach-download',['id'=>$data->file]).">$data->file</a>";

                    return "<a href=".route('attach-download',['id'=>$data->file]).">".getFilePath($data->file)['original_name']."</a>";
                }
            ...
            ->rawColumns(['name', 'batch',  'passing_year', 'address','file','change_status', 'action'])
            ->make(true);
    }
